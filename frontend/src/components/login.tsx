...
import { setAuthenticated, setUser, setPswrd } from './store'
...
export default function LoginPage() {
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        const response = await fetch('https://ride-ti-reporter-backend-r2tsfeja2q-nn.a.run.app/login', {
            method: 'POST',
        ...
        if (response.ok) {
            ...
            dispatch(setUser(getString(username)))
            dispatch(setPswrd(getString(password)))
            router.push('/home')
        } else {
            setState(0)
        }
